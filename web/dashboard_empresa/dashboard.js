...
async function loadInfluencers() {
  try {
    const response = await fetch('http://54.236.163.28:8000/influencers/');
...
async function showProfile(id) {
  try {
    const response = await fetch(`http://54.236.163.28:8000/influencer/${id}`);
    const data = await response.json();
    if (data.status === 'ok') {
...
function loadSavedInfluencers() {
  ...
  savedList.forEach(id => {
    fetch(`http://54.236.163.28:8000/influencer/${id}`)
      .then(response => response.json())
      .then(data => {
        if (data.status === 'ok') {
          const influencer = data.data;
